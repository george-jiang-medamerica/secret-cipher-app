...
import { decode, cleanEncodedMessage } from '../utils/cipherUtils';
import CipherIcon from '../components/CipherIcon';
import COLORS from '../constants/colors';

export default function DecodeScreen({ navigation }: any) {
  ...
  const handleDecode = () => {
    ...
    if (!encodedMessage.trim()) {
      setErrorMessage('Please enter an encoded message');
      return;
    }
    ...
    try {
      const cleaned = cleanEncodedMessage(encodedMessage);
      const decoded = decode(cleaned, selectedCipher);
      setDecodedMessage(decoded);
    } catch (error) {
      setErrorMessage('Failed to decode message. Make sure you have the correct cipher.');
    }
  };
  ...
  const handlePaste = async () => {
    ...
    setEncodedMessage(clipboardContent);
    setErrorMessage('');
  };
  ...
  const handleCopy = async () => {
    ...
    await Clipboard.setStringAsync(decodedMessage);
    Alert.alert('Copied', 'Decoded message copied to clipboard');
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Decode Message</Text>
      ...
      <TouchableOpacity style={styles.decodeButton} onPress={handleDecode}>
        <Text style={styles.decodeButtonText}>Decode Message</Text>
      </TouchableOpacity>

      {decodedMessage && (
        <View style={styles.resultSection}>
          <Text style={styles.resultLabel}>Your message in normal text</Text>
